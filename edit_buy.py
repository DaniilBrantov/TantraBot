...
def handle_editbuy_price(message, client, sql):
    try:
        # Сохраняем введенный ID услуги
        global editbuypriceidtovar
        editbuypriceidtovar = int(message.text)

        # Запрос новой цены
        msg = client.send_message(message.chat.id, "*Введите новую цену услуги:*", parse_mode='Markdown')
        client.register_next_step_handler(msg, handle_editbuy_price_new_price, client, sql)
    except Exception as e:
        client.send_message(message.chat.id, f'🚫 | Ошибка при запросе ID услуги: {e}')

def handle_editbuy_price_new_price(message, client, sql):
    try:
        # Получаем ID услуги и новую цену
        new_price = int(message.text)

        # Запрос подтверждения изменений
        infoshop = sql.execute(f"SELECT * FROM shop WHERE id = {editbuypriceidtovar}").fetchone()

        if not infoshop:
            client.send_message(message.chat.id, "🚫 | Услуга не найдена.")
            return

        # Создаем клавиатуру для подтверждения/отмены
        rmk = types.InlineKeyboardMarkup()
        rmk.add(
            types.InlineKeyboardButton(text='✅', callback_data='editbuynewpricetovaryes'),
            types.InlineKeyboardButton(text='❌', callback_data='editbuynewpricetovarno')
        )

        # Отправляем сообщение с подтверждением
        msg = client.send_message(
            message.chat.id,
            f"*🔰 | Подтверждение изменения цены услуги:*\n\n"
            f"ID услуги: {editbuypriceidtovar}\n"
            f"Старая цена: {infoshop[2]}\n"
        ...
        # Сохраняем новую цену в глобальной переменной
        global editbuypricetovar
        editbuypricetovar = new_price

    except Exception as e:
        client.send_message(message.chat.id, f'🚫 | Ошибка при сохранении новой цены услуги: {e}')
